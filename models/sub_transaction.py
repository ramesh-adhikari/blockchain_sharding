...
class SubTransaction:
    def __init__(self,  type: str, txn_id: int, account_no: str, account_name: str, amount: int, shard_id: int, sub_txn_id: str, txn_shard_id: int, txn_timestamp: str) -> None:
        ...
        self.txn_shard_id = txn_shard_id
    ...
    def to_message(self):
        return self.type + MESSAGE_DATA_SEPARATOR \
            ...
            + str(self.shard_id)+MESSAGE_DATA_SEPARATOR\
            + self.sub_txn_id+MESSAGE_DATA_SEPARATOR\
            + str(self.txn_shard_id)+MESSAGE_DATA_SEPARATOR\
            + self.txn_timestamp

    def change_type(self, type):
        return SubTransaction(type, self.txn_id, self.account_no, self.account_name, self.amount, self.shard_id, self.sub_txn_id, self.txn_shard_id, self.txn_timestamp)

    @staticmethod
    def from_message(message):
        message = message.split(MESSAGE_DATA_SEPARATOR)
        return SubTransaction(message[0], message[1], message[2], message[3], message[4], message[5], message[6], message[7], message[8])


def split_transaction_to_sub_transactions(transcation, txn_shard_id):
