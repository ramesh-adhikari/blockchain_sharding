...
class Transaction:
    ...
    def append_sub_transaction_to_temporary_file(txn_id, sub_txn_id, account_number, account_name, amount, shard_id):
        with lock if LOCK_FILE_TO_MAKE_THREAD_SAFE else nullcontext():
            ...
            data = [txn_id, sub_txn_id, account_number,
                    account_name, amount, timestamp]
    ...
    def move_sub_transaction_to_committed_transaction(shard_id, sub_txn_id):
        with lock if LOCK_FILE_TO_MAKE_THREAD_SAFE else nullcontext():
            ...
            transaction.move_transaction(
                source_file, destination_file, sub_txn_id, 'TRANSACTION')
            print("Moved "+sub_txn_id)  # TODO remove comment
    ...
    def remove_transaction_from_temporary_transaction(shard_id, sub_txn_id):
        ...
        t_instance.delete_row_by_txn_id(
            temporary_pool_txn_path, sub_txn_id, 'TRANSACTION')
    ...
    def is_commited_transaction(shard_id, sub_txn_id):
        abs_file_path = os.path.abspath(
            os.curdir) + FilesGenerator().get_txn_file_path(shard_id, 'committed')
        ...
        if ("SUB_TXN_ID" in data.index):  # TODO prevents key error but check if this is causing issue?
    ...
    def is_temporary_transaction(shard_id, sub_txn_id):
        abs_file_path = os.path.abspath(
            os.curdir) + FilesGenerator().get_txn_file_path(shard_id, 'temporary')
        # TODO Check pd read implementation
        data = None
        while True:
            try:
    ...
    def get_transactions_from_transaction_pool(shard_id):
        txn_pool_initial_file_name = os.path.abspath(
            os.curdir)+FilesGenerator().get_txn_pool_file_path(shard_id, 'initial')
        data = pd.read_csv(txn_pool_initial_file_name)
        # sort transaction as per timestamp
        data['TIMESTAMP'] = pd.to_datetime(
            data['TIMESTAMP'], format="%Y/%m/%d %H:%M")
        ...
        with open(txn_pool_initial_file_name, newline='') as f:
            ...
            if (len(list(reader)) > 1):
                with open(txn_pool_initial_file_name, newline='') as fn:
                    ...
                    Transaction().move_transaction_from_initial_to_temporary_pool(
                        shard_id, row2[0])
                    return row2
            else:
                return None
    ...
    def has_amount(account_number, amount):
        ...
        account_amount = data_frame.loc[data_frame['ACCOUNT_NUMBER']
                                        == account_number, 'AMOUNT'].sum()
    ...
    def move_transaction(self, source_file, destination_file, txn_id, type='TRANSACTION_POOL'):
        ...
        while True:
            try:
                transaction = pd.read_csv(
                    os.path.abspath(os.curdir)+source_file)
                break
            except:
                time.sleep(5/1000)
        ...
        if (type == 'TRANSACTION'):
            t_instance.delete_row_by_txn_id(
                source_file, selected_rows['SUB_TXN_ID'][selected_rows.index[0]], type)
        else:
            t_instance.delete_row_by_txn_id(
                source_file, selected_rows['TXN_ID'][selected_rows.index[0]], type)
    ...
    def delete_row_by_txn_id(self, file_path, txn_id, type):
        ...
        if (type == 'TRANSACTION'):
            transaction.drop(
                transaction.index[(transaction["SUB_TXN_ID"] == txn_id)], axis=0, inplace=True)
        else:
            transaction.drop(
                transaction.index[(transaction["TXN_ID"] == txn_id)], axis=0, inplace=True)
    ...
    def is_account_locked(shard_id, txn_shard_id, account_number):
        if (TRANSACTION_TYPE == 'LOCK'):
            ...
            account = data_frame.loc[data_frame['ACCOUNT_NUMBER']
                                     == account_number]
            if (len(account) > 0):
    ...
    def get_row_from_snapshot(shard_id, account_no):
        if (TRANSACTION_TYPE == 'OUR_PROTOCOL'):
            ...
            selected_row = snapshot.loc[(
                snapshot["ACCOUNT_NUMBER"] == account_no)]
            if (len(selected_row) > 0):
                return [
    ...
    def snapshot_exists(shard_id, account_no, sub_txn_id):
        if (TRANSACTION_TYPE == 'OUR_PROTOCOL'):
            abs_file_path = os.path.abspath(
                os.curdir) + FilesGenerator().get_txn_file_path(shard_id, 'snapshot')
            # TODO Check pd read implementation
            ...
            selected_row = snapshot.loc[(snapshot["ACCOUNT_NUMBER"] == account_no) & (
                snapshot["SUB_TXN_ID"] == sub_txn_id)]
            if (len(selected_row) > 0):
                return True
            else:
                return False
    ...
    def get_timestamp_from_last_row_of_committed_txn(shard_id, account_no):
        if (TRANSACTION_TYPE == 'OUR_PROTOCOL'):
            ...
            snapshot = pd.read_csv(abs_file_path)
            selected_row = snapshot.loc[(
                snapshot["ACCOUNT_NUMBER"] == account_no)].tail(1)
